...
from chanjo2.crud.intervals import get_genes, get_hgnc_gene, get_gene_intervals
...
from chanjo2.meta.handle_tasks import report_stats_multitasker
...
def get_report_data(
    ...
    gene_intervals_coords: Dict[str, List[Tuple[str, int, int]]] = get_gene_intervals_coords(genes=genes, interval_type=query.interval_type, db=session, build=query.build.value)


    report_stats_multitasker(query=query, gene_intervals_coords=gene_intervals_coords, report_data=data)
...
def get_gene_intervals_coords(genes:List[Tuple[str, int, int]], interval_type: IntervalType, db:Session, build:Builds) -> Dict[str, List[Tuple[str, int, int]]]:
    ...
    if IntervalType == IntervalType.GENES:
        return {gene.ensembl_id:[(gene.chromosome, gene.start, gene.stop)] for gene in genes}

    gene_intervals_coords: Dict[str, List[Tuple[str, int, int]]] = {gene.ensembl_id: [] for gene in genes}
    intervals: List[Union[SQLTranscript, SQLExon]] = get_gene_intervals(db=db, build=build, ensembl_gene_ids=[gene.ensembl_id for gene in genes], interval_type=INTERVAL_TYPE_SQL_TYPE[interval_type], transcript_tags=["refseq_mrna"])
    for interval in intervals:
        gene_intervals_coords[interval.ensembl_gene_id].append((interval.chromosome, interval.start, interval.stop))
